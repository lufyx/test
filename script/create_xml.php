...
// Effacer les anciens fichiers xml
$dossier_traite = "../data";
...
// Xml contenant les noms des bases (lesBases.xml)
$lesBases = new DOMDocument('1.0','UTF-8');
$lesBases->formatOutput = true; // Structurer le fichier xml

$portails = $lesBases->createElement('portails'); // Création de la balise principale "portails"
$maBase = $lesBases->appendChild($portails); // On la place dans le fichier

$tabBases = file('mesBases_3.10.txt');
$tabBases2 = file('mesBases_3.11.txt');

foreach ($tabBases as $lineNumber => $lineContent)
{
	$infoBase = explode("#", $lineContent);
	$nomDB = $infoBase[0]."dbprod";
	$labelDB = utf8_encode($infoBase[1]);
	
		$maBase = $lesBases->createElement('database');
		$maBase = $portails->appendChild($maBase);
		$maBase->setAttribute("nomBase",$nomDB);
				$texte = $lesBases->createTextNode($labelDB);
				$texte = $maBase->appendChild($texte);
}

foreach ($tabBases2 as $lineNumber => $lineContent)
{
	$infoBase = explode("#", $lineContent);
	$nomDB = $infoBase[0];
	if ($nomDB != "grandparis")
...
$lesBases->save("../data/lesBases.xml");
...
foreach ($tabBases as $lineNumber => $lineContent)
{
	...
	mysql_connect("192.168.3.10",$loginDB,$mdpDB) or die("Connexion impossible à ".$nomDB);
		...
		while($ligneCat = mysql_fetch_array($reqCat))
		{
			...
			while($ligneInfoArticle = mysql_fetch_array($reqInfoArticle))
			{
				...
				$uneDescriptionArticle = utf8_encode($ligneInfoArticle["entry_text"]);
					...
					$uneDescriptionArticle = str_replace(chr(30),"",$uneDescriptionArticle);
